...
class SLList {
    ...
    // think about what to do if the list has multiple negatives
    // Edge cases: the whole list is negative or there are no negatives

    // remove all negatives you find in the sll!
    removeNegatives() {
        if (this.head == null) { //edge case for empty SLL
            console.log("This list is empty!")
            return this
        }
        //still need edge case for if we end up skipping head or tail
        var runner = this.head
        var positive = this.head
        //what happens to the runner if it starts on a negative number? -> use removeFromFront function
        while (runner.next != null) { // gets to end of the list
            //look for next positive number in the list
            if (runner.next.value < 0) {
                // console.log('You made it!: ' + runner)
                positive = runner
                while (runner.next.value < 0) {
                    runner = runner.next;
                }
                positive.next = runner.next
            }
            runner = runner.next;
        }
        //what happens if we've skipped over the tail? -> while loop through to the end again, and set the tail equal to the runner once it's at the end? Or there's prob also way to use the removeFromBack function before we ever skip the last number.
    }
...
secondToLast() {
    ...
    var runner = this.head
    while (runner.next.next != null) { //get to one before the location (but stop once we get to the end)
        runner = runner.next; // increment to the next node in the SLL
    }
    console.log("The second to last value in the list is: " + runner.value)
}
}
